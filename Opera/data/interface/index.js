...
var downloadhub={
    ...
    getImages:function(onlyTopFrame){
        ...
        downloadhub.initiated=false;
        var allFrames=conf.allframes;
        if(allFrames)
            allFrames=!onlyTopFrame;
        downloadhub.allimages=new Object();
        downloadhub.monitorStatus(onlyTopFrame);
        //reset old results
        ...
        chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
            downloadhub.currUrl=tabs[0].url;
            chrome.scripting.executeScript({target:{tabId:tabs[0].id,allFrames: allFrames}, files:['lib/image.js']}, (res) => {downloadhub.getImagesCallback(res);})
        })
    },
        ...
        if(chrome.extension.lastError || !results){
            ...
            for(var r=0;r<results.length;r++){
                ...
                if(results[r].result.isTop)
                    downloadhub.title=results[r].result.title;
                var fixRefererRules=[];
                ...
                for(var i=0;i<images.length;i++){
                    try{
                        ...
                        if(downloadhub.allimages[images[i]]){
                            processedImages++;
                            continue;
                        }
                        downloadhub.allimages[images[i]]=images[i];
                            ...
                            div.onclick = function(event) {
                                ...
                                if(event.target.className=="zoomIcon downloadIcon"){
                                    chrome.downloads.download({
                                        url: (conf.web2jpg && this.getAttribute("convertedimgsrc"))? this.getAttribute("convertedimgsrc") : this.getAttribute("imgsrc")
                                    });
                                    return;
                                }
    ...
    selectImg:function(div,force,forceValue){
        ...
        var toSelect=force?forceValue:!downloadhub.selection[src];
        if(toSelect){
            checkbox.checked="checked";
            downloadhub.selection[src]=src;
            div.classList.add("imgSelected");
        }else{
            checkbox.checked="";
            delete downloadhub.selection[src];
            div.classList.remove("imgSelected");
        }
        var selectionLength=0;
        for (var k in downloadhub.selection) {
            if (downloadhub.selection.hasOwnProperty(k)) 
                selectionLength++;
        }
    ...
    suggestFileName:function(item,suggest){
        var title="downloadhub - " + downloadhub.title;
    ...
    init:function(){
        ...
        if(conf.hidetools)
            downloadhub.showTools(false);
        setTimeout(function(){
            $("#container")[0].style.maxWidth=document.body.offsetWidth+"px";
        },100);
        
        downloadhub.localize();
        downloadhub.initSizeMenu();
        ...
        $("#showPrefs")[0].onclick = downloadhub.showPrefs;
        
        $("#saveprefscancel")[0].onclick=downloadhub.showPrefs
        $("#saveprefs")[0].onclick=downloadhub.savePrefs       
        ...
        $("#selectalla")[0].onclick = function(){
            ...
            allimages.each(function(){
                downloadhub.selectImg(this,true,toSelect);
            })
        }
        ...
        $("#ratebutton")[0].onclick = downloadhub.openRatingTab
        $("#rateclose")[0].onclick = downloadhub.showRating
        $( ".filters > div div[saveSize]" ).on( "click", function(event) {
            var saveSizeEl=$(this);
            if(conf.savesize){
                conf.savesize=false;
                saveSizeEl.removeClass("selected");
                downloadhub.saveSizeConf(true);
            }else{
                conf.savesize=true;
    ...
    getConf:function(callback){
        chrome.storage.sync.get({
            ...
            function(items) {
                ...
                downloadhub.numviews=items.numviews+1;
                var howoftentoshowrating=downloadhub.numviews>1000?100000:downloadhub.numviews>100?1000:downloadhub.numviews>10?100:10;
                if(downloadhub.numviews%howoftentoshowrating==0){
                    downloadhub.showRating();
                }
                chrome.storage.sync.set({
                    numviews: downloadhub.numviews
                });
            }
        );
    },
    foundlastimage:function(){
        ...
        chrome.runtime.sendMessage({msg: "getImagesCT"}, function(response) {
            ...
            $(".imgContainer").each(function(){
                var div=$(this);
                downloadhub.selectImg(this,true,false);//reset old selection
                ...
                if(dimension)
                    div.find(".imgDimension").html(downloadhub.formatBytes(dimension,0));
            ...
            downloadhub.selection=new Object();//reset old selection
            ...
            $(".typeMenu").find("div[typeConf]").each(function(){
                ...
                var currTypeNum=downloadhub.stats.types[currType];
            ...
            $(".sizeMenu").find("div[sizeConf]").each(function(){
                ...
                if(currSize=="custom")
                    return
                var currSizeNum=downloadhub.stats.size[currSize];
            ...
            $(".layoutMenu").find("div[layoutConf]").each(function(){
                ...
                var currLayoutNum=downloadhub.stats.layout[currLayout];
